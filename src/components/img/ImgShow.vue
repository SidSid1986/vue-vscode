...
// ✅ 监听 img-container 宽度变化的逻辑
const observeImgContainer = () => {
  if (!imgContainerRef.value) return;

  // 创建 ResizeObserver 实例
  resizeObserver = new ResizeObserver((entries) => {
    for (const entry of entries) {
      const { width } = entry.contentRect; // 当前容器的宽度（单位：px）

      console.log(
        "[ResizeObserver] img-container 当前宽度: ",
        width.toFixed(2),
        "px"
      );
    }
  ...
  if (imgContainerRef.value) {
    resizeObserver.observe(imgContainerRef.value);
  }
};
...
const onImageLoad = () => {
  ...
  // 1. 获取原始宽高
  imageNaturalWidth = img.naturalWidth; // ✅ 记录原始宽
  imageNaturalHeight = img.naturalHeight; // ✅ 记录原始高
  ...
  const viewportWidthPx = window.innerWidth; // 当前浏览器窗口总宽度

  // 假设 图片占窗口的 49.9%（ 可以根据需求调整这个比例， 就对应原来的 49.5vw）
  // const targetWidthVw = 49.9; //

  console.log("获取的" + props.rightWidthProp);

  const targetWidthVw = props.rightWidthProp; //  后期从 props 或配置动态获取，这里先写死为了和原来逻辑对齐
  ...
  if (targetWidthPx < minDisplayWidth) {
    targetWidthPx = minDisplayWidth;
    console.log(
      `[onImageLoad] 调整后宽度过小，已限制为最小宽度: ${targetWidthPx}px`
    );
  }

  // 动态设置最大显示高度（根据 rightWidthProp 的逻辑）
  ...
  // ✅ 使用响应式的 MAX_WIDTH 和动态的 maxDisplayHeight
  const MAX_DISPLAY_WIDTH_PX = MAX_WIDTH.value; // 850 或动态调整
  const MAX_DISPLAY_HEIGHT_PX = maxDisplayHeightPx; // 550 或 650
  ...
  console.log(
    `[onImageLoad] 最终计算渲染尺寸: ${finalWidth.toFixed(
      0
    )}px x ${finalHeight.toFixed(0)}px`
  );
